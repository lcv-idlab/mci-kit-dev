<?php snippet('header-ricerca') ?>
	...
	<main>
		...
		<section>
			<h2 class="hidden home-title-container"><?php echo page('ricerca/risorse')->title()->html() ?></h2>
		...
		<?php if(page('ricerca/eventi')->isVisible()): ?>
		...
		<section>
			<header class="home-title-container">
				<hr class="hr-solid">
				<h2 class="title"><?php echo page('ricerca/eventi')->title()->html() ?></h2>
				<a href="<?php echo page('eventi')->url() ?>" class="link"><span>vedi tutti gli eventi</span></a>
			</header>
			<ul class="container-events">
				<?php foreach (page('ricerca')->children()->find('eventi')->children()->visible() as $event): ?>
					<li>
